...
type Set struct {
	ID          int64      `gorm:"primaryKey"`
	ExerciseID  int64      `gorm:"index"`
	Reps        int        `gorm:"type:integer"`
	FactReps    int        `gorm:"type:integer"`
	Weight      float32    `gorm:"type:real"`
	FactWeight  float32    `gorm:"type:real"`
	Minutes     int        `gorm:"type:integer"`
	FactMinutes int        `gorm:"type:integer"`
	Completed   bool       `gorm:"type:boolean"`
	CompletedAt *time.Time `gorm:"type:timestamp with time zone"`
	Index       int        `gorm:"type:integer"`
}
